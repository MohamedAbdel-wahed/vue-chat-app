<template>
   <div id="message" >
       <div class="fixed w-full md:w-8/12 -mt-48 h-20 bottom-0 flex items-center bg-white">
           ...
           <button class="ml-2 md:ml-4 focus:outline-none opacity-50" :class="[active ? 'opacity-100' : 'opacity-25']" @click="addMessage()">
    ...
    methods:{
       addMessage(){
           if(this.message!==null){
               db.collection('users').where('alias','==',this.$route.params.alias).get().then(result=>{
